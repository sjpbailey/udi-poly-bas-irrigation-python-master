...
class BasIrrigationController(Controller):
    ...
    def query(self,command=None):
        ...
        for node in self.nodes:
            self.nodes[node].reportDrivers()
    class bc:
        def __init__(self, sIpAddress, ePlatform):  #sIpAddress
            self.bc = Device()
    ...
    def check_params(self):
        ...
        if 'irr1_ip' and 'irr2_ip' and 'irr3_ip' and 'irr4_ip' and 'irr5_ip' and 'irr6_ip'in self.polyConfig['customParams']:
            ...
            LOGGER.error(
                'check_params: The First BASpi6u6r IP is not defined in customParams, please add it.  Using {}'.format(self.ipaddress))
            st = False        
    ...
    def discover(self, *args, **kwargs):
        ...
        if self.ipaddress is not None:
            self.addNode(BaspiNodeOne(self, self.address, 'baspi1_id', 'Zone Control One', self.ipaddress, self.bc))
            self.setDriver('GV19', 1, force=True)
        if self.ipaddress2 is not None:
                self.addNode(BaspiNodeTwo(self, self.address, 'baspi2_id', 'Zone Control Two', self.ipaddress2, self.bc2))
                self.setDriver('GV20', 1, force=True)
        if self.ipaddress3 is not None:
                self.addNode(BaspiNodeThree(self, self.address, 'baspi3_id', 'Zone Control Three', self.ipaddress3, self.bc3))
                self.setDriver('GV21', 1, force=True)
        if self.ipaddress4 is not None:
                self.addNode(BaspiNodeFour(self, self.address, 'baspi4_id', 'Zone Control Four', self.ipaddress4, self.bc4))
                self.setDriver('GV22', 1, force=True)
        if self.ipaddress5 is not None:
                self.addNode(BaspiNodeFive(self, self.address, 'baspi5_id', 'Zone Control Five', self.ipaddress5, self.bc5))
                self.setDriver('GV23', 1, force=True)
        if self.ipaddress6 is not None:
                self.addNode(BaspiNodeSix(self, self.address, 'baspi6_id', 'Zone Control Six', self.ipaddress6, self.bc6))
                self.setDriver('GV24', 1, force=True)
        return
        ##elif a.isdigit():
        ##    if self.ipaddress is not None:
        ##        self.addNode(BaspiNodeOne(self, self.address, 'baspi1_id', 'Zone Control One', self.ipaddress, self.bc))
        ##        self.setDriver('GV19', 1, force=True)
        ###    elif self.ipaddress2 is not None:
        ##        self.addNode(BaspiNodeTwo(self, self.address, 'baspi2_id', 'Zone Control Two', self.ipaddress2, self.bc2))
        ##        self.setDriver('GV20', 1, force=True)
        ##    elif self.ipaddress3 is not None:
        ##        self.addNode(BaspiNodeThree(self, self.address, 'baspi3_id', 'Zone Control Three', self.ipaddress3, self.bc3))
        ##        self.setDriver('GV21', 1, force=True)
        ##    elif self.ipaddress4 is not None:
        ##        self.addNode(BaspiNodeFour(self, self.address, 'baspi4_id', 'Zone Control Four', self.ipaddress4, self.bc4))
        ##        self.setDriver('GV22', 1, force=True)
        ##    elif self.ipaddress5 is not None:
        ##        self.addNode(BaspiNodeFive(self, self.address, 'baspi5_id', 'Zone Control Five', self.ipaddress5, self.bc5))
        ##        self.setDriver('GV23', 1, force=True)
        ##    elif self.ipaddress6 is not None:
        ##        self.addNode(BaspiNodeSix(self, self.address, 'baspi6_id', 'Zone Control Six', self.ipaddress6, self.bc6))
        ##        self.setDriver('GV24', 1, force=True)
        ##    else:
        ##        pass
        ##else:
        ##    pass
        LOGGER.info('AddNode is complete')
    ...
    drivers = [
        ...
        {'driver': 'GV21', 'value': 1, 'uom': 2},
        {'driver': 'GV22', 'value': 1, 'uom': 2},
        {'driver': 'GV23', 'value': 1, 'uom': 2},
        {'driver': 'GV24', 'value': 1, 'uom': 2},
    ]
