...
class BaspiNodeFour(polyinterface.Node):
    ...
    def start(self):
        ...
        elif self.bc4.ePlatform == Platform.BASC_ED: ### if a BASpi-Edge Device is found
            LOGGER.info('connected to BASpi-Edge')
            self.setDriver('ST', 1)    
        elif self.bc4.ePlatform == Platform.BASC_NONE: ### if there is NO Device found
        ...
        LOGGER.info('\t' + str(self.bc4.uiQty) + ' Universal inputs in this Device')
        LOGGER.info('\t' + str(self.bc4.boQty) + ' Binary outputs in this Device')
        LOGGER.info('\t' + str(self.bc4.vtQty) + ' Virtual points In This Device')        
    ...
    def cmdOn(self, command):
        ...
        if self.bc4.binaryOutput(index) != 1:
            self.bc4.binaryOutput(index, 1)
    ...
    def cmdOff(self, command):
        output = self.mapping[command['cmd']]['output']
        index = self.mapping[command['cmd']]['index']
        if self.bc4.binaryOutput(index) != 0:
            self.bc4.binaryOutput(index, 0)
