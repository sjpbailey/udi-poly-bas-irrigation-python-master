...
class BaspiNodeThree(polyinterface.Node):
    def __init__(self, controller, primary, address, name, ipaddress, bc3):
        super(BaspiNodeThree, self).__init__(controller, primary, address, name)
    ...
    def start(self):
        if self.ipaddress is not None:
            self.bc3 = Device(self.ipaddress)
                  
        ### Do we have a BASpi or an Edge Device ###
        if self.bc3.ePlatform == Platform.BASC_PI or self.bc3.ePlatform == Platform.BASC_PO: ### if a BASpi-6u6r Device is found
            LOGGER.info('connected to BASpi-6U6R')
        elif self.bc3.ePlatform == Platform.BASC_ED: ### if a BASpi-Edge Device is found
            LOGGER.info('connected to BASpi-Edge')
            self.setDriver('ST', 1)    
        elif self.bc3.ePlatform == Platform.BASC_NONE: ### if there is NO Device found
        ...
        LOGGER.info('\t' + str(self.bc3.uiQty) + ' Universal inputs in this Device')
        LOGGER.info('\t' + str(self.bc3.boQty) + ' Binary outputs in this Device')
        LOGGER.info('\t' + str(self.bc3.vtQty) + ' Virtual points In This Device')        
        
        # Input/Output Status
        LOGGER.info('Inputs')
        for i in range(1,7):
               LOGGER.info(str(self.bc3.universalInput(i)))
        LOGGER.info('Outputs')
        for i in range(1,7):
               LOGGER.info(str(self.bc3.binaryOutput(i)))
    ...
    def setInputDriver(self, driver, input):
        input_val = self.bc3.universalInput(input)
        count = 0
    ...
    def setOutputDriver(self, driver, input):
        output_val = self.bc3.binaryOutput(input)
        count = 0
    ...
    def cmdOn(self, command):
        ...
        if self.bc3.binaryOutput(index) != 1:
            self.bc3.binaryOutput(index, 1)
    ...
    def cmdOff(self, command):
        output = self.mapping[command['cmd']]['output']
        index = self.mapping[command['cmd']]['index']
        if self.bc3.binaryOutput(index) != 0:
            self.bc3.binaryOutput(index, 0)
