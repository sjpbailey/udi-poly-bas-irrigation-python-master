...
class BaspiNodeTwo(polyinterface.Node):
    def __init__(self, controller, primary, address, name, ipaddress, bc2):
        super(BaspiNodeTwo, self).__init__(controller, primary, address, name)
    ...
    def start(self):
        if self.ipaddress is not None:
            self.bc2 = Device(self.ipaddress)
                  
        ### Do we have a BASpi or an Edge Device ###
        if self.bc2.ePlatform == Platform.BASC_PI or self.bc2.ePlatform == Platform.BASC_PO: ### if a BASpi-6u6r Device is found
        ...
        LOGGER.info('\t' + str(self.bc2.uiQty) + ' Universal inputs in this Device')
        LOGGER.info('\t' + str(self.bc2.boQty) + ' Binary outputs in this Device')
        LOGGER.info('\t' + str(self.bc2.vtQty) + ' Virtual points In This Device')        
        ...
        if input_six is not None:
            sumss_count = int(float(self.bc2.universalInput(6))) 
        else:
            return
  
    ### Universal Input Conversion ###
    def setInputDriver(self, driver, input):
        input_val = self.bc2.universalInput(input)
        count = 0
    ...
    def setOutputDriver(self, driver, input):
        output_val = self.bc2.binaryOutput(input)
        count = 0
