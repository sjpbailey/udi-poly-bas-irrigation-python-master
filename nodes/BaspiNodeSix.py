...
class BaspiNodeSix(polyinterface.Node):
    def __init__(self, controller, primary, address, name, ipaddress, bc6):
        super(BaspiNodeSix, self).__init__(controller, primary, address, name)
    ...
    def start(self):
        ...
        if self.bc6.ePlatform == Platform.BASC_PI or self.bc6.ePlatform == Platform.BASC_PO: ### if a BASpi-6u6r Device is found
            LOGGER.info('connected to BASpi-6U6R')
        elif self.bc6.ePlatform == Platform.BASC_ED: ### if a BASpi-Edge Device is found
            LOGGER.info('connected to BASpi-Edge')
            self.setDriver('ST', 1)    
        elif self.bc6.ePlatform == Platform.BASC_NONE: ### if there is NO Device found
        ...
        LOGGER.info('\t' + str(self.bc6.uiQty) + ' Universal inputs in this Device')
        LOGGER.info('\t' + str(self.bc6.boQty) + ' Binary outputs in this Device')
        LOGGER.info('\t' + str(self.bc6.vtQty) + ' Virtual points In This Device')        
        
        # Input/Output Status
        LOGGER.info('Inputs')
        for i in range(1,7):
            LOGGER.info(str(self.bc6.universalInput(i)))
        LOGGER.info('Outputs')
        for i in range(1,7):
            LOGGER.info(str(self.bc6.binaryOutput(i)))
    ...
    def setInputDriver(self, driver, input):
        input_val = self.bc6.universalInput(input)
        count = 0
    ...
    def setOutputDriver(self, driver, input):
        output_val = self.bc6.binaryOutput(input)
        count = 0
    ...
    def cmdOn(self, command):
        ...
        if self.bc6.binaryOutput(index) != 1:
            self.bc6.binaryOutput(index, 1)
    ...
    def cmdOff(self, command):
        output = self.mapping[command['cmd']]['output']
        index = self.mapping[command['cmd']]['index']
        if self.bc6.binaryOutput(index) != 0:
            self.bc6.binaryOutput(index, 0)
